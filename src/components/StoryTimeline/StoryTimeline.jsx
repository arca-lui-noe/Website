    ...
    useEffect(() => {
        ...
        const setupTimelineAnimation = () => {
            ...
            tempImage.onload = () => {
                ...
                imageWidth = tempImage.naturalWidth * (imageHeight / tempImage.naturalHeight);
            ...
            tempImage.onerror = () => {
                console.error("Failed to load 'story_timeline.webp'. Please ensure the image exists in the public directory.");
        ...
        const handleKeyDown = (e) => {
            if (e.key === 'ArrowLeft' && leftBtnRef.current && !leftBtnRef.current.disabled) {
                e.preventDefault();
                navigateTimeline(-1);
            } else if (e.key === 'ArrowRight' && rightBtnRef.current && !rightBtnRef.current.disabled) {
                e.preventDefault();
                navigateTimeline(1);
            }
        };
        ...
        return () => {
            ...
            if (leftBtnRef.current) {
                leftBtnRef.current.removeEventListener('click', () => navigateTimeline(-1));
            }
            if (rightBtnRef.current) {
                rightBtnRef.current.removeEventListener('click', () => navigateTimeline(1));
            }
                ...
                <div className="timeline-container">
                    ...
                    <div className="section_title">
                        <h2 className="title_text">
                            <span className="sub_title">Our History</span> Interactive Timeline
                        </h2>
                    </div>
</div>
                    <div className="position-relative">
                        ...
                        <div className="timeline-controls left">
                            <button className="btn-control" id="control-timeline-left" aria-label="Scroll left" ref={leftBtnRef}>
                                <ChevronLeft size={32} />
                            </button>
                        </div>

                        <div className="timeline-controls right">
                            <button className="btn-control" id="control-timeline-right" aria-label="Scroll right" ref={rightBtnRef}>
